...
// newTestApplication returns an instance of application struct containing mocked dependencies
func newTestApplication(t *testing.T) *application {
	...
	session := sessions.New([]byte("urveiuo4fo446sdvsdcADNJKdc"))
	session.Lifetime = 12 * time.Hour
	session.Secure = true

	// Initialize the dependencies, using the mocks the loggers and
	// database models.
	return &application{
...
// get method on our custom testServer type.
// This makes a GET request to a given url path on the test server, and returns the response
