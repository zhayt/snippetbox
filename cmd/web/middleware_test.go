...
func TestSecureHeaders(t *testing.T) {
	// Initialize a new httptest.ResponseRecorder and any dummy http.Request
	...
	if frameOptions != "deny" {
		t.Errorf("wants %q; got %q", "deny", frameOptions)
	}
	...
	if xssProtection != "1; mode=block" {
		t.Errorf("wants %q; got %q", "1; mode=block", xssProtection)
	}

	// Check that the middleware has correctly called the next handler in line
	// and the response status code and body are as expected.
	if rs.StatusCode != http.StatusOK {
		t.Errorf("wants %d; got %d", http.StatusOK, rs.StatusCode)
	}
	...
	if string(body) != "OK" {
		t.Errorf("wants body to equal %q", "OK")
	}
}
